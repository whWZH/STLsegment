...
public:
	...
	union {
		struct {
			float center[3];   // the middle number of x, y, z
			float r;
			int splitaxis;        // 0, 1, 2 is x, y, z 
			Node *child1, *child2;
		} node;
		struct {
	...
	void find_pt_in_Range(Traversal_Info &ti) const;
...
void KDtree::Node::find_pt_in_Range(KDtree::Node::Traversal_Info &ti)const
{
	// Leaf nodes
	if (npts)
	{
		for (int i = 0; i < npts; i ++)
		{
			float myd2 = dist2(leaf.p[i], ti.p);  // leaf to p   
			if (myd2 < ti.closest_d2  && (!ti.iscompat || (*ti.iscompat)(leaf.p[i])))
			{
				float myd = sqrt(myd2);
				ti.knn.push_back(make_pair(myd, leaf.p[i]));
	...
	// Recursive case
	float myd = node.center[node.splitaxis] - ti.p[node.splitaxis];   // p to center
	if (myd >= 0.0f) {
		node.child1->find_pt_in_Range(ti);
		//if (myd < ti.closest_d)
		node.child2->find_pt_in_Range(ti);
	} else {
		node.child2->find_pt_in_Range(ti);
		//if (-myd < ti.closest_d)
		node.child1->find_pt_in_Range(ti);
...
// Find the points within the range around pt
void KDtree::find_pt_in_Range(std::vector<const float *> &knn, 
	                                        const float *p, 
											float maxdist2, 
											const CompatFunc *iscompat /* = NULL */)const
{
	...
	ti.iscompat = iscompat;
	ti.closest = NULL;
	ti.closest_d2 = maxdist2;       // R * R 
	ti.closest_d = sqrt(ti.closest_d2);

	root->find_pt_in_Range(ti);
